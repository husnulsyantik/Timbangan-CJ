...
namespace CJTimbangan.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        public ReportController(SettingTimbanganContext context)
        {
            db = context;
        }
        ...
        public async Task<IActionResult> Index(int? page)
        {
            ...
            var model = qry.ToPagedList(page ?? 1, 10);
            //var model = qry;
            return View(model);
        ...
        [HttpGet]
        public async Task<IActionResult> Lysine(int page = 1)
        {
            var qry = db.m_code.AsNoTracking().Where(p => p.barang == "Lysine").OrderByDescending(p => p.id);
            var model = await PagingList.CreateAsync(qry, 10, page);
            return View(model);
        }
        [HttpGet]
        public async Task<IActionResult> Cuka(int page = 1)
        {
            var qry = db.m_code.AsNoTracking().Where(p => p.barang == "Cuka").OrderByDescending(p => p.id);
            var model = await PagingList.CreateAsync(qry, 10, page);
            return View(model);
        }
